...
## Приёмка задач и Trello (v0)

После обработки планёрки предложенные ИИ задачи попадают на экран **Приёмка** (`http://localhost:8080/inbox`, кнопка «📥 Приёмка» в шапке):

- **Задачи** (есть исполнитель, уверенность высокая) — правишь текст, выбираешь исполнителя из списка, вписываешь критерий готовности (обязателен), ставишь срок одной кнопкой («Сегодня-завтра / На этой неделе / До конца месяца» — считаются **от даты планёрки**) и жмёшь «Отправить в Trello».
- **Мелочи** (прозвучало, но поручением не было) — лежат при встрече, в Trello не уходят, пока не нажмёшь «＋ В задачи».
- **Имена спикеров** назначаются там же и не блокируют приёмку.
- Кнопка **🎧** у задачи играет запись с таймкода цитаты (аудио сохраняется рядом с артефактами встречи).

Trello — строго в одну сторону: карточка создаётся в списке «К работе», статусы дальше живут на доске. **Первый срок** каждой задачи пишется в журнал (SQLite, `due_v1` защищён триггером) и зеркалится сообщением в Telegram — его не переписать, даже если дату на доске подвинут.

Настройка — в `.env`: `TRELLO_KEY`, `TRELLO_TOKEN`, `TRELLO_LIST_ID` (+ опционально `TRELLO_BOARD_ID` для меток/участников), `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`. Подробности — в `.env.example`.

**Бот просрочек** (`scripts/overdue_bot.py`) раз в день читает доску и шлёт одно сообщение со всеми просроченными карточками. Он живёт в GitHub Actions (`.github/workflows/overdue-bot.yml` — секреты в Settings → Secrets → Actions), потому что локальный комп бывает выключен; локальный запуск `python scripts/overdue_bot.py --dry-run` — для проверки.

**Дата планёрки** больше не равна дате обработки: берётся из поля формы (предзаполняется из имени файла или даты изменения), для воронки — из имени файла/mtime. Десять старых записей, прогнанных за вечер, больше не слипаются в один день.

Решения зафиксированы в `docs/adr/001` (задачи живут в Trello), `docs/adr/002` (SQLite в named-томе вместо Postgres), ТЗ — `docs/tz-v0.md`. Тесты: `python -m unittest discover tests`.
